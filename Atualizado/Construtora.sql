...
INSERT INTO funcionarios_projetos (id_funcionario,id_projeto,horas_trabalhadas,valor_hora) VALUES
	...
	select count(distinct fp.id_projeto)
	...
	select p.nome as projeto, f.nome as funcionario, sum(fp.horas_trabalhadas) as horas_trabalhadas
	from projetos p 
	inner join funcionarios_projetos fp on p.id_projeto = fp.id_projeto
	...
	select p.nome as projeto, count(fp.id_funcionario)
	from projetos p
